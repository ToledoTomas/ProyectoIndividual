...
const url5 = await axios.get(`https://api.rawg.io/api/games?key=${API_KEY}&page=5`)
    ...
    arrayGames = url1.data.results.concat(
        url2.data.results,
        url3.data.results,
        url4.data.results,
        url5.data.results,
    ...
    arrayGames = arrayGames.map((game) => {
    ...
    const dataBase = async () => {
      ...
      const total = gamesData.concat(dbInfo);
      return total;
    };
...
router.post('/', async (req, res) => {
    ...
    res.status(200).send('Successfully created video game');
});    
